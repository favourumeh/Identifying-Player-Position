...
ma = GNB.score(X_test, y_test) # accuracy = 0.608(unscaled data)
...
print(f'For 5 fold cross validation: The mean mean accuracy = {ma_cv:.3f} ') # 0.603(unscaled)  
...
for c,i in enumerate(best_variables):
    ...
    GNB2 = GaussianNB()
    acc = cross_val_score(GNB2, X_train_drop, y_train.values.flatten(), cv = 5, scoring = 'accuracy').mean()
    ...
    X_train_drop = X_train_drop.drop(i, axis = 1)  
        ...
        #from this anlysis the most important variables are:
        #['FGA', 'FG_per', '3P', '3PA', '3P_per', '2P', '2PA', '2P_per', 
...
#Model 
X_train = X_train[['FGA', 'FG_per', '3P', '3PA', '3P_per', '2P', '2PA', '2P_per', 'FTA', 'FT_per', 'ORB', 'DRB', 'TRB', 'AST', 'STL', 'BLK', 'TOV', 'PF']]
GNB3 = GaussianNB().fit(X_train, y_train.values.flatten()) 

    #1)  test data
    
X_test = X_test[['FGA', 'FG_per', '3P', '3PA', '3P_per', '2P', '2PA', '2P_per', 'FTA', 'FT_per', 'ORB', 'DRB', 'TRB', 'AST', 'STL', 'BLK', 'TOV', 'PF']] 
y_pred_test1 =  GNB3.predict(X_test) 


ma3b = GNB3.score(X_test, y_test)# 0.616 >0.608(untuned)
print(f'The mean accuracy of the test model(hold-out) in is : {ma3b:.3f}')


cm3 = confusion_matrix(y_test, y_pred_test1)

plt.figure()
plot_confusion_matrix(cm3,classes = GNB.classes_, title='GNB_tuned: Confusion matrix for test data')

print(classification_report(y_test, y_pred_test1))

    #2) Evaluation of 10%

X_eval = df_10.drop('Pos', axis = 1)[['FGA', 'FG_per', '3P', '3PA', '3P_per', '2P', '2PA', '2P_per', 'FTA', 'FT_per', 'ORB', 'DRB', 'TRB', 'AST', 'STL', 'BLK', 'TOV', 'PF']]

y_eval = df_10.Pos

y_pred_eval = GNB3.predict(X_eval)

ma3b = GNB3.score(X_eval,y_eval)# 0.628
print(f'The mean accuracy of the testing evaluation model(hold-out) in is : {ma3b:.3f}')

cm3 = confusion_matrix(y_eval, y_pred_eval)

plt.figure()
plot_confusion_matrix(cm3,classes = GNB.classes_, title='GNB_tuned: Confusion matrix for evaluation data')
...
# with open('Optimal_GNB_model_train.sav','wb') as f:
#       pickle.dump(GNB3,f)
...
Pred_prob_df = pd.DataFrame(GNB3.predict_proba(X_test), columns = ['GNB_'+ i for i in list(GNB3.classes_)])   
